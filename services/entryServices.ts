...
const createEntry = (request, response) => {
  ...
  Database.query('INSERT INTO entries (uid, dates, entry) VALUES ($1, $2, $3)', [uid, dates, entry], (error, _results) => {
    if (error) {
      throw error
    }
...
const deleteEntry = (request, response) => {
  ...
  Database.query('DELETE FROM entries WHERE uid = $1 AND dates = $2', [uid, dates], (error, _results) => {
    if (error) {
      throw error
    }
